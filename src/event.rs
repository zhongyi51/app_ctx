...
impl ErasedListener {
    ...
    pub(crate) fn sync_one<E: 'static>(
        listener: impl AppEventListener<EventType = E> + Sync + Send + 'static,
    ) -> Self {
        let inner =
            Arc::new(Box::new(listener)
                as Box<
                    dyn AppEventListener<EventType = E> + Sync + Send + 'static,
                >);
        return Self {
            is_async: false,
    ...
    pub(crate) fn async_one<E: 'static>(
        listener: impl AppEventListenerAsync<EventType = E> + Sync + Send + 'static,
    ) -> Self {
        let inner = Arc::new(Box::new(listener)
            as Box<dyn AppEventListenerAsync<EventType = E> + Sync + Send + 'static>);
        return Self {
            is_async: true,
            event_type: type_name::<E>(),
            inner,
    ...
    pub(crate) fn deal_sync<T: 'static + Send>(
        &self,
        event: AppEvent<T>,
        executor: &impl Deref<Target = dyn EventExecutor + Sync + Send>,
    ) {
        let r = self
            .inner
            .clone()
    ...
    pub(crate) async fn deal_async<T: 'static + Send>(
        &self,
        event: AppEvent<T>,
        executor: &impl Deref<Target = dyn EventExecutorAsync + Sync + Send>,
    ) {
        let r = self
            .inner
            .clone()
...
pub trait AppEventListener {
    ...
    fn on_event(&self, event: AppEvent<Self::EventType>);
}
...
pub trait AppEventListenerAsync {
    ...
    async fn on_event_async(&self, event: AppEvent<Self::EventType>);
}
